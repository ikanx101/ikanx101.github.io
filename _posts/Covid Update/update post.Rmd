---
title: "PSBB Level II: PSBBT (PEMBATASAN SOSIAL BERSKALA BESAR TOTAL) ~ Tambah India dan Amerika"
output: 
  github_document:
    pandoc_args: --webtex
---
...
rm(list=ls())
setwd("~/ikanx101com/_posts/Covid Update")
options(scipen = 99)
library(dplyr)
library(ggplot2)
library(ggpubr)

data = read.csv("~/belajaR/Bukan Infografis/Covid Data/owid-covid-data.csv")

target = c("Indonesia","Italy","Malaysia","Singapore","New Zealand","China")
...
Sekarang saya coba membujuk Anda, kenapa sih __PSBBT__ (panjang amat sih istilahnya. Coba ditulis __lockdown__ aja gitu!?) harus segera dilakukan di Indonesia yah! (minimal di DKI Jakarta dan sekitarnya).

Berbekal data yang dihimpun di situs [_Our World in Data_](https://ourworldindata.org/coronavirus) per pagi ini (14 September 2020 pukul 07.00 WBI), saya akan coba membandingkan data negara kita dengan data negara lainnya seperti: `Italy`, `Singapura`, `Malaysia`, `New Zealand`, dan `China`.

> Kenapa sih dibandingkan dengan `5` negara tersebut?

Mereka adalah negara yang menerapkan _lockdown_ pada saat awal-awal pandemi. Mari kita bandingkan dengan `Indonesia` yang tidak menerapkan _lockdown_!
...
data %>% 
  ...
  labs(title = "Cumulative Confirmed Cases for Selected Country",
       subtitle = "Akankah Indonesia melampaui Italia?",
       x = "Tanggal",
...
Di saat negara-negara pilihan ini sudah berkurang penambahan kasus hariannya, Indonesia masih terus naik dan hampir setiap hari memecahkan rekor. Kemarin, Indonesia baru saja memecahkan rekor penambahan kasus tertinggi lagi!
...
Tadi pagi saya mendengar berita bahwa ada pejabat yang mewaspadai gelombang kedua dari COVID-19 di Indonesia.

Kalau menurut saya pribadi:

> Kita belum selesai gelombang I _lho_!
...
data %>% 
  ...
  geom_line(aes(group = location,color = location)) +
  labs(title = "Positive Rate for Selected Country",
       subtitle = "Jangan bangga kita bisa di atas kedua negara ini",
       x = "Tanggal",
       y = "Banyak Orang yang Positif\ndari Total Orang yang di Test",
       caption = "Scraped and Visualized\nUsing R\nikanx101.com") +
  theme_pubclean() +
