...
_Total cases_ adalah angka _real infected person_ di suatu negara. Secara total dunia, sampai saat data ini saya _scrape_, sudah ada `r round(sum(show_data$total_cases)/1000000,2)` juta orang yang terinfeksi Covid 19. Jika kita lihat sebaran `ratio_sakit` semua negara, kita bisa melihat:

```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.retina=10}
clean_data %>%
  ggplot(aes(x = ratio_sakit)) +
  geom_density()

clean_data %>%
  ggplot(aes(y = ratio_sakit)) +
  geom_boxplot()
```
