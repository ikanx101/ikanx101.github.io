...
Kita bisa _save_ rute tersebut menjadi file `.kml` agar bisa dimasukkan ke dalam *Google Earth* atau *Google Mymaps*, caranya:

```{r}
st_write(obj = mytrip, dsn = "tes rute.kml", driver = "libkml")
```

Berikut adalah _screenshot_ setelah saya _upload_ ke *Google Mymaps*:
